...
class [[gnu::packed]] FileSystem{
    public:
        ...
        virtual uint64_t read(void* buffer, uint64_t length, uint64_t skip = 0) = 0;
        virtual uint64_t write(void* buffer, uint64_t length, uint64_t skip = 0) = 0;
...
class [[gnu::packed]] DriverFile : public FileSystem{
    public:
        ...
        uint64_t read(void* buffer, uint64_t length, uint64_t skip = 0){
            ...
            mutex->release();
            uint64_t read_length = drv->devRead(buffer, length);
        ...
        uint64_t write(void* buffer, uint64_t length, uint64_t skip = 0){
            ...
            uint64_t write_length = drv->devWrite(buffer, length);
