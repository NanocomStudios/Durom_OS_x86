...
void pmm_init(){
    ...
    page_frames_start = (page_alloc_bitmap_end  + 0x1000) & (0xFFFFFFFFFFFFF000);

    page_alloc_bitmap = (uint64_t*)(DEFAULT_HHDM_OFFSET + ramBase);
...
void* page_alloc(){
    for(uint64_t i = 0; i < ceiling_div(page_count, 64); i++){
        if(page_alloc_bitmap[i] != (uint64_t)0xFFFFFFFFFFFFFFFF){
            for(int j = 0; j < 8; j++){
                if(((char*)page_alloc_bitmap)[i*8 + j] != (char)0xFF){
                    for(int k = 0; k < 8; k++){
                        if((((char*)page_alloc_bitmap)[i*8 + j] & (1 << k)) == 0){
                            ((char*)page_alloc_bitmap)[i*8 + j] |= (1 << k);
                            uint64_t pageAddr = page_frames_start + ((i * 64 + j * 8 + k) * 0x1000);
